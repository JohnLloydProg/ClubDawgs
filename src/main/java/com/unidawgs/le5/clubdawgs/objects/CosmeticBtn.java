...
public class CosmeticBtn extends ImgBtn {
    ...
    private final Image bgImage = new Image(Main.class.getResource("cosmeticContainer.png").toString());
    ...
    public CosmeticBtn(int row, int col, Image image, int cosmetic, boolean owned) {
        super(163 + (col * 110), 290 + (row * 110), 90, 90, image, Color.GRAY);
        this.owned = owned;
    ...
    public void draw(GraphicsContext gc, double offset) {
        ...
        gc.drawImage(this.bgImage, this.xPos, this.yPos, this.width, this.height);
